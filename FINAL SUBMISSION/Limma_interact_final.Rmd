...
```{r}
Top_10_genes <-as.data.frame(rownames(topTable(nfit2, coef = "interact",  adjust="BH")))
Top_10_genes
...
Top_10_genes$GeneSymbol = mapIds(org.Hs.eg.db,
                    keys=rownames(topTable(nfit2, coef = "interact",  adjust="BH")), #Column containing Ensembl gene ids
                    column="SYMBOL",
                    keytype="ENSEMBL",
                    multiVals="first") #This selects the first gene alias, if there are multiple gene names under the single EntrezID
Top_10_genes
top_table <-as.data.frame(topTable(nfit2, coef = "interact",  adjust="BH"))
top_table
...
```{r}
volcanoplot(nfit2, "interact", highlight = 10, main="Zonal 'interaction' Treatment Across Week1-Week2")#The highlight=10 highlights the top 10, but gives the rownames... Changing to Ensembl gene name truncates the name, so it is not useful. However, we can capture the row name and get the gene name, but why bother, as the names are already in the top10 gene list above from the toptable() function. So see above, we made a fancy table with the gene SYMBOL line 351 ^)
```
...
pheatmap(interact_sig_hclust_g1,annotation_col = annotation, scale="row", cluster_cols = F, show_rownames = F, main = "Zonal (W1EGCG - W1Placebo) - (W2EGCG - W2Placebo) Cluster Group #1 (k=4)" )
...
keggEnrich <- enrichKEGG(
  ...
  keyType = "kegg",
  qvalueCutoff = 0.45,
  pvalueCutoff = 1, #adjust this if you are not seeing any results
  pAdjustMethod = "BH",
  )
```
...
pv.out_htmp1a <- pathview(gene.data = diffexpgenes_names_df$entrez, pathway.id = "hsa04141", species = "hsa")
#Repeat for the second result
#pv.out_htmp1b <- pathview(gene.data = diffexpgenes_names_df$entrez, pathway.id = "hsa00000", species = "hsa")
```
...
pv.out_htmp3a <- pathview(gene.data = diffexpgenes_names_df$entrez, pathway.id = "hsa04810", species = "hsa")
#Repeat for the second result
pv.out_htmp3b <- pathview(gene.data = diffexpgenes_names_df$entrez, pathway.id = "hsa04144", species = "hsa")
```
...
pv.out_htmp4a <- pathview(gene.data = diffexpgenes_names_df$entrez, gene.idtype = "KEGG", pathway.id = "hsa05219", species = "hsa")
#Repeat for the second result
pv.out_htmp4b <- pathview(gene.data = diffexpgenes_names_df$entrez, gene.idtype = "KEGG", pathway.id = "hsa05213", species = "hsa")
```
